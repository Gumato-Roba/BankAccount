...
class Account:
    ...
    def borrow(self):
        ...
        if len(self.deposits) <10:
            return f"Hello {self.accname}  you are not qualified to borrow.make {len(self.deposits)} to borrow "
        elif self.amount<100:
            return f"Hello {self.accname} you are not qualified to borrow a loan"
        elif self.amount>sum/3:
            return f"Hello {self.accname} you can only borrow upto {sum/3}"
        # elif self.balance >0:
        #     return f"Hello  {self.accname} you have {self.balance} you can't borrow any loan and you have a debt "
    ...
    def transfer(self,amount,new_account):
        if amount<=0:
            ...
            return f'Successfully sent to {amount} to {self.accname} and your balance is {self.balance}'
        if amount>self.balance:
          return f"Hello {self.accname} you have insuficient amounts to do the transfer"